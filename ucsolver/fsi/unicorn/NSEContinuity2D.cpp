                                    ...
                                    const ufc::cell& c) const
{
    ...
    const static double P_t1_p0_s1[1][2] = \
    {{-1, 1}};
    // Array of non-zero columns
    static const unsigned int nzc0[2] = {0, 2};
    // Array of non-zero columns
    ...
    const static double P_t0_p0[1][3] = \
    {{0.333333333333333, 0.333333333333333, 0.333333333333333}};
    
    // Number of operations to compute geometry constants = 32
    const double G0 = Jinv_00*W0*det;
    const double G1 = Jinv_01*W0*det;
    const double G2 = Jinv_10*W0*det;
    const double G3 = Jinv_11*W0*det;
    const double G4 = Jinv_00*Jinv_00*W1*det*w[2][0];
    const double G5 = Jinv_00*Jinv_10*W1*det*w[2][0];
    const double G6 = Jinv_01*Jinv_01*W1*det*w[2][0];
    const double G7 = Jinv_01*Jinv_11*W1*det*w[2][0];
    const double G8 = Jinv_10*Jinv_10*W1*det*w[2][0];
    const double G9 = Jinv_11*Jinv_11*W1*det*w[2][0];
    ...
    {
      F0 += P_t1_p0_s1[0][r]*w[1][nzc2[r]];
      F1 += P_t1_p0_s1[0][r]*w[1][nzc5[r]];
      F2 += P_t1_p0_s1[0][r]*w[1][nzc3[r]];
      F3 += P_t1_p0_s1[0][r]*w[1][nzc4[r]];
      F4 += P_t1_p0_s1[0][r]*w[0][nzc1[r]];
      F5 += P_t1_p0_s1[0][r]*w[0][nzc0[r]];
    }// end loop over 'r'
    
    // Number of operations to compute declarations = 17
    const double Gip0 = F0*G0 + F1*G1 + F2*G2 + F3*G3;
    const double Gip1 = (G5 + G7)*F5 + (G4 + G6)*F4;
    const double Gip2 = (G8 + G9)*F5 + (G5 + G7)*F4;
    ...
    {
      // Number of operations to compute entry = 2
      A[nzc1[j]] += P_t1_p0_s1[0][j]*Gip1;
      // Number of operations to compute entry = 2
      A[nzc0[j]] += P_t1_p0_s1[0][j]*Gip2;
    }// end loop over 'j'
...
{
    return "-w1_a0[0, 1, 2, 3, 4, 5](dXa1[0, 1]/dxa2[0, 1]) | vi0[0, 1, 2]*((d/dXa1[0, 1])va0[0, 1, 2, 3, 4, 5][a2[0, 1]])*dX(0) + 2.0w2_a0[0]w0_a1[0, 1, 2](dXa2[0, 1]/dxb0[0, 1])(dXa3[0, 1]/dxb0[0, 1]) | va0[0]*((d/dXa2[0, 1])vi0[0, 1, 2])*((d/dXa3[0, 1])va1[0, 1, 2])*dX(0)";
}
